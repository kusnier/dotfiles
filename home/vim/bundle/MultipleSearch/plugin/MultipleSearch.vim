...
if v:version < 700
    ...
    vnoremap <silent> <Leader><Leader>* y:call MultipleSearch(0,'\V'.substitute(escape(@@,"\\/\"'"),"\n",'\\n','ge'))<CR>
else
    ...
    " Following Manuel's idea, adapt the former 'Super Star' tip from vim.org to work with
    " :Search on a visual selection.
    vnoremap <silent> <Leader><Leader>* y:call MultipleSearch#MultipleSearch(0,'\V'.substitute(escape(@@,"\\/\"'"),"\n",'\\n','ge'))<CR>
endif

" Thanks to Manuel Picaza for the following mapping to :Search the word under
" the cursor.
nnoremap <silent> <Leader><Leader>* :execute ':Search \<' . expand('<cword>') . '\>'<cr>
