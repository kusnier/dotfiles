...
" Version:  $Id: xpcomplete.vim 709 2010-07-15 10:58:05Z seek $
"
"   TODO:
...
"     best situation
"
"   XP-Framework support
"   TODO:
"   - Add support for return values, detect type of the assignment
...
function! xpcomplete#CompletePHP(findstart, base)
	...
	if !exists('g:php_builtin_functions')
		call xpcomplete#LoadData()
	endif

	let scontext = substitute(context, '\$\?[a-zA-Z_\x7f-\xff][a-zA-Z_0-9\x7f-\xff]*$', '', '')
	if scontext =~ '\(=\s*new\|extends\)\s\+$'
    let final_menu = xpcomplete#GetClassMenu(a:base)
    return final_menu
	elseif scontext =~ '\::$'
    let final_menu = xpcomplete#GetClassMember(a:base)
    return final_menu
	elseif scontext =~ '\(->\|::\)$'
		...
		if scontext =~ '->$' && scontext !~ '\$this->$'
			...
			let classname = xpcomplete#GetClassName(scontext)
			if classname != ''
        if a:base =~ '->'
          let member = matchstr(a:base, '->\zs[a-zA-Z_0-9\x7f-\xff]\+\ze')
        else
          let member = matchstr(a:base, '\zs[a-zA-Z_0-9\x7f-\xff]\+\ze')
        endif

        let final_menu = xpcomplete#GetClassMember(classname.'->'.member)
        if len(final_menu) > 0
          return final_menu
        endif
      endif
			...
			if classname != ''
				let classlocation = xpcomplete#GetClassLocation(classname)
			else
				let classlocation = ''
			endif
			...
			if filereadable(classlocation)
				...
				let classcontent = ''
				let classcontent .= "\n".xpcomplete#GetClassContents(classfile, classname)
    ...
    let class_menu = xpcomplete#GetClassMenu(a:base)
    let final_list = extend(final_list, class_menu)
...
function! xpcomplete#GetClassMenu(name) " {{{
	" Get class menu
		" Complete class name
		...
		let fnames = join(map(tagfiles(), 'escape(v:val, " \\#%")'))
		if fnames != ''
			exe 'silent! vimgrep /^'.a:name.'.*\tc\(\t\|$\)/j '.fnames
			let qflist = getqflist()
			if len(qflist) > 0
				for field in qflist
					...
					" checked by ctags.
					let item = matchstr(field['text'], '^[^[:space:]]\+')
					let ext_classes[item] = field['text']
				endfor
			endif
		endif
		...
		for key in classes
			if key =~ '^'.a:name
        if has_key(int_classes, key)
          let info = int_classes[key]
          let final_menu += [{'word':key, 'kind':'c', 'menu':info}]
        endif
        if has_key(ext_classes, key)
          let info = xpcomplete#GetCtagsDefinition(ext_classes[key])
          let namespace = xpcomplete#GetCtagsXPNamespace(ext_classes[key])
          let final_menu += [{'word':key, 'kind':'c', 'menu':namespace, 'info':info}]
        endif
        if has_key(g:php_omni_bi_classes, key)
          let info = g:php_omni_bi_classes[key]
          let final_menu += [{'word':key, 'kind':'c', 'menu':info}]
        endif
			endif
		endfor
...
function! xpcomplete#GetClassTree(classlist) " {{{
    let currentclass= a:classlist[len(a:classlist)-1]
    let fnames = join(map(tagfiles(), 'escape(v:val, " \\#%")'))
    if fnames != ''
      exe 'silent! vimgrep /^.*\t.*\/'.currentclass.'\.class\.php.*class.*\(extends\|implements\).*\t[c]\(\t\|$\)/j '.fnames
      let qflist = getqflist()
      if len(qflist) > 0
        let field = qflist[0]
        let parentclass = matchstr(field['text'], 'class\s\+\w\+\s\+extends\s\+\zs\w\+\ze\s')
        let implements = matchstr(field['text'], 'class\s\+\w\+.*implements\s\+\zs\(\w\+\s*,\?\s*\)\+\ze\s')
        if len(implements) > 0
          let parentclasslist = [parentclass] + split(implements)
        else
          let parentclasslist = [parentclass]
        endif
        let parents = xpcomplete#GetClassTree(a:classlist + parentclasslist)
        return parents
      else
        return a:classlist
      endif
...
function! xpcomplete#GetClassMember(name) " {{{
	" Get class member menu
  " TODO: ex_classes use index(item) with full namespace
    let classname = matchstr(a:name, '^\(.*\.\|\)\zs\w\+')
    let namespace = matchstr(a:name, '^\zs\(\w\|\.\)*\ze\.\w\+')
    let nspath = substitute(namespace, '\.', '\\/', '')
    let member = substitute(a:name, '.*\(->\|::\)', '', '')
    let classlist = xpcomplete#GetClassTree([classname])
		...
		if fnames != ''
      if len(nspath) > 0
        exe 'silent! vimgrep /^.*\t.*\/'.nspath.'\/'.classname.'\.class\.php.*\t[fv]\(\t\|$\)/j '.fnames
      else
        exe 'silent! vimgrep /^.*\t.*\/'.classname.'\.class\.php.*\t[fv]\(\t\|$\)/j '.fnames
      endif
      if len(classlist) > 1
        exe 'silent! vimgrepadd /^.*\t.*\/\('.join(classlist, '\|').'\)\.class\.php.*\t[fv]\(\t\|$\)/j '.fnames
      endif
			let qflist = getqflist()
			if len(qflist) > 0
				for field in qflist
        ...
        if has_key(ext_classes, key)
          let info = xpcomplete#GetCtagsDefinition(ext_classes[key])
          let namespace = xpcomplete#GetCtagsXPNamespace(ext_classes[key])
          let type = xpcomplete#GetCtagsType(ext_classes[key])
          if a:name =~ '::'
            let fullname = classname.'::'.key
          else
            let fullname = key
          endif
          if type == 'f'
...
" }}}
"
function! xpcomplete#GetCtagsXPNamespace(ctagsline) " {{{
  " Get class name space from ctags
  let namespace = a:ctagsline
  let namespace = matchstr(namespace, '[^[:space:]]\+.class.php')
  let namespace = substitute(namespace, '^.*\(skeleton\|src\|ports\/classes\)\/', '', '')
  let namespace = substitute(namespace, '\.class\.php$', '', '')
  let namespace = substitute(namespace, '\/', '.', 'g')
  return namespace
endfunction

function! xpcomplete#GetCtagsDefinition(ctagsline) " {{{
  " Get class definition from ctags
  let definition = a:ctagsline
  "let definition = substitute(definition, '^.*\^', '', '')
  let definition = matchstr(definition, '\^.*{') " TODO: use \zs and \ze, see let objects...
  let definition = substitute(definition, '^\^[[:blank:]]*', '', '')
  let definition = substitute(definition, '[[:blank:]]*{$', '', '')
  return definition
endfunction

function! xpcomplete#GetCtagsType(ctagsline) " {{{
  " Get the ctags type
  let type = matchstr(a:ctagsline, '\t\zs[fv]\ze$') " TODO: use \zs and \ze, see let objects...
  return type
endfunction
...
function! xpcomplete#GetClassName(scontext) " {{{
  ...
  let object = matchstr(a:scontext, '\(.*(\zs.\+\ze\|.*\)') " cut nested calls
    ...
    if line =~ '@param\s\+[\.a-zA-Z_0-9\x7f-\xff]\+\s\+'.object.'\($\|\s\)'
      let classname = matchstr(line, '@param\s\+\zs[\.a-zA-Z_0-9\x7f-\xff]\+\ze\s\+'.object)
      return classname
    endif
    if line =~ '\$'.object.'\s*=\s*new\s\+\w\+\s*('
      let classname = matchstr(line, '\$'.object.'\s*=\s*new\s\+\zs\w\+\ze\s*(')
      return classname
    endif
	endwhile
...
" }}}
function! xpcomplete#GetClassLocation(classname) " {{{
	...
	for fname in tagfiles()
		...
		if classlocation != ''
      if !filereadable(classlocation)
        " If file not exists use it as relative path
...
function! xpcomplete#GetClassContents(file, name) " {{{
	...
	bw! %
	if extends_class != ''
		let classlocation = xpcomplete#GetClassLocation(extends_class)
		if filereadable(classlocation)
			let classfile = readfile(classlocation)
			let classcontent .= "\n".xpcomplete#GetClassContents(classfile, extends_class)
		endif
	endif
...
"
" TODO: comment this line out, only for development used
""set completefunc=xpcomplete#CompletePHP
